        ...
        if (result === undefined) {
            ...
            if (!result.active){
                ...
                if (!bcrypt.compareSync(body.password, result.password)) {
                    ...
                    _result.message = messagesapp.user_login_successful
                    const userNotices = userModel.getNotices(result);

                    if (userNotices) {
                        _result.notices = userNotices.notices
                    }
                    res.status(200).json(_result);
                }
            }
...
const addProfileData = (req, res, next) => {
    try {
        ...
        if (result === undefined) {
            ...
            delete _result.password;
            delete _result.active
            _result.message = messagesapp.user_added_profile
